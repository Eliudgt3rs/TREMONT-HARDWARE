...
import { Search, ShoppingCart, Menu, X, Phone, Mail, MapPin, Trash, Grid, List, ChevronDown, User, ArrowRight, ShieldCheck, Truck, Zap, LogIn, Package, FileText, CreditCard, Clock, Facebook, Instagram, Twitter, CheckCircle, AlertCircle, Smartphone, Star, SlidersHorizontal } from 'lucide-react';
...
export default function ProductsPage() {
  ...
  if (!phoneRegex.test(cleanPhone) && !phoneRegex.test('254' + cleanPhone.slice(1))) {
  ...
  try {
    const response = await fetch('/api/mpesa/stkpush', {
      ...
      body: JSON.stringify({
        ...
        amount: cartTotal,
        accountReference: currentInvoice?.invoiceNumber || 'ORDER-' + Date.now(),
  ...
  }, [searchQuery, selectedCategories, selectedBrands, priceRange, selectedRating, sortBy, inStockOnly, currentPage, productsPerPage]);

  useEffect(() => {
    setCurrentPage(1);
  }, [searchQuery, selectedCategories, selectedBrands, priceRange, selectedRating, sortBy, inStockOnly]);
  ...
  return (
    <div className="min-h-screen flex flex-col">
          ...
          <div className="container mx-auto flex justify-between items-center">
            ...
            <div className="flex items-center space-x-4">
              ...
              <div className="relative">
                ...
                >
                  {isLoggedIn && user ? user.name : 'My Account'} <ChevronDown size={14} className="ml-1" />
                </button>
                ...
                {isUserMenuOpen && (
                  <div className="absolute right-0 mt-2 bg-white text-gray-800 rounded-lg shadow-lg py-2 w-48 z-10">
                    {isLoggedIn ? (
                      <>
                        <a href="#" className="block px-4 py-2 hover:bg-gray-100 items-center" onClick={() => {setShowOrders(true); setIsUserMenuOpen(false);}}>
                          <Package size={16} className="mr-2" /> My Orders
                        </a>
                        <a href="#" className="px-4 py-2 hover:bg-gray-100 flex items-center">
                          <User size={16} className="mr-2" /> Profile Settings
                        </a>
                        <div className="border-t border-gray-200 my-1"></div>
                        <a href="#" className="block px-4 py-2 hover:bg-gray-100 text-red-600 items-center" onClick={handleLogout}>
                          <LogIn size={16} className="mr-2" /> Logout
                        </a>
                      </>
                    ) : (
                      <>
                        <a href="#" className="px-4 py-2 hover:bg-gray-100 flex items-center" onClick={() => {setShowLoginForm(true); setIsUserMenuOpen(false);}}>
                          <LogIn size={16} className="mr-2" /> Login
                        </a>
                        <a href="#" className="block px-4 py-2 hover:bg-gray-100 items-center" onClick={() => {setShowRegisterForm(true); setIsUserMenuOpen(false);}}>
          ...
          <div className="flex justify-between items-center">
            ...
            <div className="hidden md:flex font-semibold items-center space-x-8">
              <a href="./" className="hover:text-yellow-400">Home</a>
                <a href="./products" className="hover:text-white text-yellow-400">Products</a>
      ...
      {/* Page Title */}
      <section className="bg-ima text-white py-8" style={{
            ...
            <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
              ...
              <div className="flex items-center gap-1">
                ...
                >
                  ...
                  Filters
                  {(selectedCategories.length > 0 || selectedBrands.length > 0 || selectedRating > 0 || inStockOnly) && (
                    <span className="bg-yellow-500 text-gray-900 text-xs font-bold rounded-full h-5 w-5 flex items-center justify-center">
                      {selectedCategories.length + selectedBrands.length + (selectedRating > 0 ? 1 : 0) + (inStockOnly ? 1 : 0)}
                    </span>
                  )}
                </button>
                ...
                <div className="flex items-center gap-1">
                  <button
                    ...
                    className={`p-2 rounded ${viewMode === 'grid' ? 'bg-yellow-500 text-gray-900' : 'bg-gray-200 text-gray-700'}`}
                  >
                    <Grid size={18} />
                  </button>
                  <button
                    onClick={() => setViewMode('list')}
                    className={`p-2 rounded ${viewMode === 'list' ? 'bg-yellow-500 text-gray-900' : 'bg-gray-200 text-gray-700'}`}
                  >
                    <List size={18} />
                  </button>
                </div>
            ...
            {(selectedCategories.length > 0 || selectedBrands.length > 0 || selectedRating > 0 || inStockOnly) && (
              <div className="flex items-center gap-2 mt-4 pt-4 border-t border-gray-200">
                ...
                {selectedCategories.map(category => (
                  <span key={category} className="inline-flex items-center gap-1 px-3 py-1 bg-yellow-100 text-yellow-800 text-sm rounded-full">
                    {category}
                    <button onClick={() => handleCategoryChange(category)} className="hover:text-yellow-600">
                      <X size={14} />
                    </button>
                  </span>
                ))}
                {selectedBrands.map(brand => (
                  <span key={brand} className="inline-flex items-center gap-1 px-3 py-1 bg-blue-100 text-blue-800 text-sm rounded-full">
                    {brand}
                    <button onClick={() => handleBrandChange(brand)} className="hover:text-blue-600">
                      <X size={14} />
                    </button>
                  </span>
                ))}
                {selectedRating > 0 && (
                  <span className="inline-flex items-center gap-1 px-3 py-1 bg-green-100 text-green-800 text-sm rounded-full">
                    ...
                    <button onClick={() => setSelectedRating(0)} className="hover:text-green-600">
                      <X size={14} />
                    </button>
                  </span>
                )}
                {inStockOnly && (
                  <span className="inline-flex items-center gap-1 px-3 py-1 bg-purple-100 text-purple-800 text-sm rounded-full">
                    In Stock Only
                    <button onClick={() => setInStockOnly(false)} className="hover:text-purple-600">
                      <X size={14} />
                    </button>
                  </span>
                )}
            ...
            <div className={`${isFilterOpen ? 'lg:w-3/4' : 'w-full'}`}>
              ...
              <div className="flex justify-between items-center mb-6">
                <p className="text-gray-600">
                  {loading ? 'Loading...' : `Showing ${paginatedProducts.length} of ${totalProducts} products`}
              ...
              {!loading && !error && viewMode === 'grid' && products.length > 0 && (
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                  {paginatedProducts.map(product => (
                    <div key={product.id} className="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition duration-300 hover:-translate-y-2">
                      <div className="relative">
                      ...
                      <div className="p-4">
                        ...
                        <div className="flex justify-between items-center">
                          <div className="flex flex-col w-full">
                            ...
                            {product.originalPrice && (
                              <span className="text-sm text-gray-500 line-through">KSh {product.originalPrice.toFixed(2)}</span>
                            )}
                          </div>
              ...
              {!loading && !error && viewMode === 'list' && products.length > 0 && (
                <div className="space-y-4">
                  {paginatedProducts.map(product => (
                    <div key={product.id} className="bg-white rounded-4xl shadow-md overflow-hidden hover:shadow-lg transition duration-300">
                        ...
                        <div className="p-4 sm:w-2/3 lg:w-3/4">
                          ...
                          <div>
                            <div className="flex flex-col">
                              ...
                              {product.originalPrice && (
                                <span className="text-lg text-gray-500 line-through">KSh {product.originalPrice.toFixed(2)}</span>
                              )}
                            </div>
                            <button 
                              onClick={() => addToCart(product)}
      ...
      {isCartOpen && (
        <div className="fixed inset-0 z-50 flex justify-end">
          <div className="fixed inset-0 bg-gray-700 bg-opacity-100" onClick={toggleCart}></div>
  ...
  try {
    ...
    alert("Failed to generate PDF. Please try again.");
            ...
            <div id='invoiceModal' className="border-2 z-10 rounded-lg bg-white mb-6">
                  ...
                  <tbody>
                    {currentInvoice.items.map((item, index) => (
                      <tr key={index} className="border-b">
                        ...
                        <td className="py-3 px-4">KSh {(item.product.price).toFixed(2)}</td>
                        <td className="py-3 px-4 text-right">KSh {(item.product.price * item.quantity).toFixed(2)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
      ...
      {showLoginForm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          <div className="fixed inset-0 bg-black bg-opacity-50" onClick={() => setShowLoginForm(false)}></div>
          <div className="relative bg-white rounded-lg shadow-xl max-w-md w-full p-6">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-2xl font-bold">Login</h2>
              <button onClick={() => setShowLoginForm(false)} className="p-1 hover:bg-gray-100 rounded-full">
                <X size={20} />
              </button>
            </div>
            ...
            <form onSubmit={handleLoginSubmit}>
              ...
              <div className="flex items-center justify-between mb-6">
                ...
                <a href="#" className="text-sm text-yellow-600 hover:text-yellow-800">Forgot password?</a>
              </div>
      ...
      {showRegisterForm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          <div className="fixed inset-0 bg-black bg-opacity-50" onClick={() => setShowRegisterForm(false)}></div>
          <div className="relative bg-white rounded-lg shadow-xl max-w-md w-full p-6">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-2xl font-bold">Create Account</h2>
              <button onClick={() => setShowRegisterForm(false)} className="p-1 hover:bg-gray-100 rounded-full">
                <X size={20} />
              </button>
            </div>
            ...
            <form onSubmit={handleRegisterSubmit}>
              ...
              <div className="mb-6">
                <label className="flex items-center">
                  <input type="checkbox" className="form-checkbox h-4 w-4 text-yellow-500" required />
                  <span className="ml-2 text-sm text-gray-700">
                    I agree to the <a href="#" className="text-yellow-600 hover:text-yellow-800">Terms of Service</a> and <a href="#" className="text-yellow-600 hover:text-yellow-800">Privacy Policy</a>
                  </span>
                </label>
              </div>
              <div className="flex items-center justify-between">
      ...
      {showCheckout && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          <div className="fixed inset-0 bg-black bg-opacity-50" onClick={() => setShowCheckout(false)}></div>
          <div className="relative bg-white rounded-lg shadow-xl max-w-2xl w-full p-6 max-h-screen overflow-auto">
            <div className="flex justify-between items-center mb-6">
            ...
            <div className="flex items-center justify-center mb-8">
              <div className={`flex items-center ${checkoutStep >= 1 ? 'text-yellow-600' : 'text-gray-400'}`}>
                <div className={`w-8 h-8 rounded-full flex items-center justify-center ${checkoutStep >= 1 ? 'bg-yellow-500 text-white' : 'bg-gray-300'}`}>1</div>
                <span className="ml-2 hidden sm:block">Details</span>
              </div>
              <div className={`w-12 h-0.5 mx-4 ${checkoutStep >= 2 ? 'bg-yellow-500' : 'bg-gray-300'}`}></div>
              <div className={`flex items-center ${checkoutStep >= 2 ? 'text-yellow-600' : 'text-gray-400'}`}>
                <div className={`w-8 h-8 rounded-full flex items-center justify-center ${checkoutStep >= 2 ? 'bg-yellow-500 text-white' : 'bg-gray-300'}`}>2</div>
                ...
                <div className="flex justify-end">
                  <button
                    onClick={() => {
                      ...
                      const isValid = requiredFields.every(field => checkoutForm[field as keyof typeof checkoutForm].trim() !== '');
              ...
              <div>
                <div className="text-center mb-6">
                  <div className="bg-green-50 border border-green-200 rounded-lg p-6 mb-6">
                    ...
                    <h3 className="text-lg font-bold text-green-800 mb-2">Pay with M-Pesa</h3>
                    <p className="text-green-700 text-sm">
                      Enter your M-Pesa phone number below to complete your payment securely
                    </p>
                  </div>
                </div>
                ...
                <div className="space-y-4 mb-6">
                  <div>
                    <label className="block text-gray-700 text-sm font-bold mb-2">M-Pesa Phone Number *</label>
                    <input
                      type="tel"
          ...
          <div className="relative bg-white rounded-lg shadow-xl max-w-md w-full p-6">
            <div className="text-center">
              ...
              <div className="mb-6">
                ...
                <h2 className="text-2xl font-bold text-gray-800 mb-2">M-Pesa Payment</h2>
                <p className="text-gray-600">Complete your payment using M-Pesa</p>
              </div>
              ...
              <div className="bg-gray-50 rounded-lg p-4 mb-6">
                <div className="flex justify-between items-center mb-2">
                  ...
                  <span className="font-bold text-lg">KSh {currentInvoice.total.toFixed(2)}</span>
                </div>
                <div className="flex justify-between items-center mb-2">
              ...
              <div className="space-y-3">
                {!mpesaStatus && (
                  ...
                  >
                    {isProcessingPayment ? 'Initiating Payment...' : 'Complete Payment'}
                  </button>
                )}
      ...
      {showOrderConfirmed && confirmedOrder && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          <div className="fixed inset-0 bg-black bg-opacity-50" onClick={() => setShowOrderConfirmed(false)}></div>
          <div className="relative bg-white rounded-lg shadow-xl max-w-lg w-full p-6">
            <div className="text-center">
              ...
              <div className="bg-gray-100 rounded-lg p-4 mb-6">
                ...
                <p><strong>Order ID:</strong> {confirmedOrder.id}</p>
                <p><strong>Total:</strong> KSh {confirmedOrder.total.toFixed(2)}</p>
              ...
              <div className="bg-blue-100 rounded-lg p-4 mb-6">
                <div className="flex items-center justify-center mb-2">
                  ...
                  <span className="font-semibold text-blue-800">Email Notification Sent</span>
                </div>
                <p className="text-sm text-blue-700">
                  An order confirmation has been sent to the business owner at eliaqmaa98@gmail.com. 
                  Your order will be processed within 1-3 business hours.
                </p>
              </div>
      ...
      {showOrders && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          <div className="fixed inset-0 bg-black bg-opacity-50" onClick={() => setShowOrders(false)}></div>
          <div className="relative bg-white rounded-lg shadow-xl max-w-4xl w-full p-6 max-h-screen overflow-auto">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-2xl font-bold">My Orders</h2>
              <button onClick={() => setShowOrders(false)} className="p-1 hover:bg-gray-100 rounded-full">
                <X size={20} />
              </button>
            </div>
                  ...
                  <div key={index} className="border rounded-lg overflow-hidden">
                    ...
                    <div className="p-4">
                      ...
                      <div className="flex justify-between items-center pt-2 border-t">
                        ...
                        <p className="font-bold text-lg">KSh {order.total.toFixed(2)}</p>
                      </div>
                    </div>
                  </div>
                ))}
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="flex items-center justify-center">
              ...
              <div>
                ...
                <p className="text-sm text-gray-600">On orders over KSh100,000</p>
              </div>
            </div>
            <div className="flex items-center justify-center">
              ...
              <div>
                ...
                <p className="text-sm text-gray-600">100% secure M-Pesa checkout</p>
              </div>
            </div>
            <div className="flex items-center justify-center">
